...
export default function SalonDetails() {
    ...
    return (
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-4">
          ...
          <Link href="/salons" className="px-6 py-2 rounded-full bg-rosegold-500 text-white font-semibold text-xs">
            Back to Directory
          </Link>
        </div>
        <Footer />
  ...
  const filteredServices = salon.services.filter(s => s.category === selectedCategory);
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2">
            <button 
              ...
              className={`p-2 rounded-xl border border-rosegold-200 dark:border-charcoal-800 transition-colors ${
                isSaved ? 'text-rosegold-550 bg-rosegold-50/50 dark:bg-charcoal-900' : 'text-charcoal-600 dark:text-rosegold-100 hover:bg-rosegold-50'
              }`}
              title="Save Salon"
            >
            ...
            <button 
              className="p-2 rounded-xl border border-rosegold-200 dark:border-charcoal-800 text-charcoal-600 dark:text-rosegold-100 hover:bg-rosegold-50 transition-colors"
              title="Share Profile"
            >
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[280px] sm:h-[400px]">
          ...
          <div className="hidden md:flex flex-col gap-4">
            ...
            <div className="flex-1 rounded-2xl overflow-hidden relative border border-rosegold-200/50">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img 
                src={salon.gallery[2] || salon.gallery[0] || salon.image} 
        ...
        <section className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-rosegold-200/40 dark:border-charcoal-800 pb-6">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-3xl font-bold text-charcoal-950 dark:text-white">{salon.name}</h1>
            ...
            <p className="text-xs sm:text-sm text-charcoal-550 dark:text-rosegold-250 flex items-center">
            ...
            <div className="flex items-center space-x-6 text-sm">
              <div className="flex items-center text-rosegold-500">
                ...
                <span className="text-xs text-charcoal-450 ml-1">({salon.reviewsCount} verified reviews)</span>
              </div>
          ...
          <div className="w-full md:w-auto p-4 rounded-xl border border-rosegold-150 dark:border-charcoal-800 bg-rosegold-50/10 dark:bg-charcoal-950/20 shrink-0 space-y-2.5">
            ...
            <div className="flex flex-col space-y-1.5 text-xs text-charcoal-600 dark:text-rosegold-100">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-6">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {categories.map((cat) => (
                      <button
                        ...
                        className={`text-xs px-4 py-2 rounded-full border transition-all shrink-0 ${
                    ...
                    {filteredServices.map((service) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-end shrink-0 gap-2">
                          <span className="text-sm font-bold text-charcoal-900 dark:text-white">₹{service.price}</span>
                          <Link
                            href={`/booking?salon=${salon.id}&service=${service.id}`}
                            className="px-4 py-1.5 rounded-lg bg-rosegold-500 hover:bg-rosegold-600 text-xs font-bold text-white flex items-center gap-1 shadow-2xs"
                          >
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center pb-2 border-b border-rosegold-100 dark:border-charcoal-800/80">
                    ...
                    <Link
                      ...
                      className="text-xs font-semibold text-rosegold-500 hover:text-rosegold-650 flex items-center"
                    >
                      Write Review
              ...
              {/* ABOUT TAB */}
              {activeTab === 'about' && (
                <div className="p-6 rounded-2xl border border-rosegold-150 dark:border-charcoal-800 bg-white dark:bg-charcoal-900/60 space-y-4">
                  <h3 className="font-bold text-charcoal-950 dark:text-white text-base">About {salon.name}</h3>
                  ...
                  <div className="pt-4 border-t border-rosegold-100 dark:border-charcoal-800/80 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-charcoal-650 dark:text-rosegold-150">
          ...
          <aside className="lg:col-span-1 space-y-6">
            ...
            <div className="p-6 rounded-2xl border border-rosegold-350 dark:border-charcoal-800 bg-linear-to-b from-rosegold-100/20 to-white dark:from-charcoal-900 dark:to-charcoal-950 space-y-5 shadow-sm">
              ...
              <div className="space-y-4 text-xs leading-relaxed text-charcoal-750 dark:text-rosegold-200">
                <p className="font-light italic bg-white/60 dark:bg-charcoal-950/50 p-3.5 rounded-xl border border-rosegold-100 dark:border-charcoal-900">
                  {salon.aiReviewSummary.summary}
                </p>

                {/* Popular Services Section (V2 added) */}
                {salon.aiReviewSummary.popularServices && (
                ...
                <div className="space-y-1.5">
                  <h4 className="font-semibold text-amber-700 dark:text-amber-350">Cons:</h4>
                  <ul className="space-y-1 pl-3 list-disc">
            ...
            <div className="p-5 rounded-2xl border border-rosegold-200 dark:border-charcoal-850 bg-white dark:bg-charcoal-900/60 text-center space-y-3">
        ...
        <section className="space-y-4 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {similarSalons.map((simSalon) => {
              const startingPrice = Math.min(...simSalon.services.map(s => s.price));
              return (
                <div 
                  key={simSalon.id}
                ...
                >
                  ...
                  <div className="flex sm:flex-col items-start sm:items-end justify-between sm:justify-start w-full sm:w-auto shrink-0 gap-2 pt-2 sm:pt-0">
                    <span className="text-xs text-charcoal-550">Starts from ₹{startingPrice}</span>
                    <Link
                      href={`/salons/${simSalon.id}`}
                      className="px-4 py-2 rounded-xl bg-rosegold-500 hover:bg-rosegold-650 text-xs font-bold text-white flex items-center gap-1 shadow-2xs"
                    >
                      View Profile
