...
export default function AdminPage({ defaultTab = 'overview' }: { defaultTab?: 'overview' | 'salons' | 'bookings' | 'reviews' | 'services' | 'users' }) {
  ...
  const handleSalonFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSalon) {
        await updateSalon(salonFormData.id, salonFormData, selectedImageFile, imagePreviewUrl || undefined);
      } else {
        await addSalon(salonFormData, selectedImageFile, imagePreviewUrl || undefined);
      }
                  ...
                  {imagePreviewUrl ? (
                    <div className="w-16 h-16 rounded-xl border border-border overflow-hidden relative shrink-0">
                      ...
                      <button
                        ...
                        className="absolute inset-0 bg-black/40 text-white font-bold text-xs flex items-center justify-center hover:opacity-100 opacity-0 transition-opacity cursor-pointer"
                      >
                        Remove
                      </button>
                    </div>
