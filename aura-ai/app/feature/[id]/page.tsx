        ...
        <header className="h-20 bg-white border-b border-border flex items-center justify-between px-8 flex-shrink-0 shadow-xs z-10">
          <div className="flex items-center gap-4">
            <h2 className="text-xl font-serif font-bold text-darktext capitalize">
              {featureId === 'intent' ? 'Intent Analyzer' : 
               featureId === 'recommendation' ? 'Salon Matchmaker' : 
               featureId === 'journey' ? 'Beauty Planner' : 
               featureId === 'review' ? 'Review Intelligence' : 
               featureId === 'booking' ? 'Booking Concierge' : 'Feature Showcase'}
            </h2>
            <span className="px-3 py-1 bg-peach/10 text-peach text-xs font-bold rounded-lg border border-peach/20 uppercase tracking-wide">
              Isolated Mode
            </span>
          </div>
            ...
            <div className="bg-white border-b border-border p-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {featureId === 'intent' ? 'Type a natural language request, and I will extract the core intent and parameters without acting on them.' : 
                   featureId === 'recommendation' ? 'Provide criteria like budget, location, and service, and I will rank the best matching salons.' : 
                   featureId === 'journey' ? 'Tell me about an upcoming event (e.g., Wedding in 3 weeks), and I will generate a multi-step timeline.' : 
                   featureId === 'review' ? 'Ask me to compare two salons, and I will extract pros, cons, and sentiments from user reviews.' : 
                   featureId === 'booking' ? 'Ask to schedule an appointment, and I will validate the date, time, and generate a draft booking payload.' : 
                   'Test this isolated feature.'}
                  </p>
                </div>
              </div>
              {isTyping && (
